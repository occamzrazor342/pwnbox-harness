...
`<ORCHESTRATION_ROOT>/queues/htb.md`) or `keep-working-overthewire` (queue:
`<ORCHESTRATION_ROOT>/queues/overthewire.md`). If `<ORCHESTRATION_ROOT>/worker-state.json`
...
Also create an `orchestrations` entry in `<ORCHESTRATION_ROOT>/registry.json`:
...
1. Read `<ORCHESTRATION_ROOT>/worker-state.json`. Update `lastTickAt`.
2. **If `stopRequested`**: append a `worker_stopped` line to
   `<ORCHESTRATION_ROOT>/timeline.jsonl`, set the worker's `orchestrations`
...
`<ORCHESTRATION_ROOT>/worker-state.json` for current active/paused/stopped state
and `consecutiveBlockers`, and `<ORCHESTRATION_ROOT>/registry.json` for the
...
`<ORCHESTRATION_ROOT>/worker-state.json` as specified above assumes a single
