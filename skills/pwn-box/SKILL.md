...
Before doing anything else, confirm the target falls within this project's
`SCOPE_DOC` (see `HARNESS.md`). If the target is ambiguous or clearly
outside that scope, stop and ask before dispatching any agent.

## Registry Bookkeeping

`<ORCHESTRATION_ROOT>/registry.json` tracks every goal (a goal = one target run
...
`goals` array of `<ORCHESTRATION_ROOT>/registry.json` (id: `<target-slug>-<date>`,
...
to `<ORCHESTRATION_ROOT>/timeline.jsonl`.
...
output from `<NOTES_ROOT>/Machines/`, so run them strictly in order and let each
...
1. **Dispatch `connect-agent`** with the target (HTB machine name, OTW
   ...
   Wait for it to write `<NOTES_ROOT>/Machines/<target>-connect.md`. If it reports
...
2. **Dispatch `recon-agent`** with the same target. Wait for it to write
   `<NOTES_ROOT>/Machines/<target>-recon.md`.
3. **Dispatch `exploit-agent`** with the same target. It reads the recon
   ...
   `<NOTES_ROOT>/Machines/<target>-foothold.md`. If it reports no viable vector
...
- A stage hits a genuine session/resource ceiling it cannot work around
  ...
  missing, a safety-classifier block on the instance — per this harness's
  own `docs/Agent-Operating-Principles.md`, verify these rather than
...
2. **MANDATORY synthesis checkpoint — invoke the `synthesize-state` skill
   before re-dispatching anything.** A "no viable vector" block is exactly
   the moment inherited conclusions calcify into false premises (see that
   skill's Odyssey worked example). Run it in the orchestrator's own
   context — do NOT skip it and do NOT delegate it to a subagent — so the
   next pass is built from an evidence-checked Assumption Register
   (VALIDATED vs. UNVALIDATED) rather than from the flattened prose of the
   report that just blocked. The UNVALIDATED assumptions and ranked
   Unresolved Signals it produces are what the next recon/exploit prompt is
   built from. If the synthesis reveals the block rested on discarded or
   never-tested evidence (only in `/tmp`, redacted, never recorded), the
   next action is to *re-collect and actually test that*, which often
   unsticks the goal without a generic "look harder" pass at all.
3. Dispatch `recon-agent` again for the same target, with the `model`
   ...
   synthesis's UNVALIDATED assumptions / Unresolved Signals (step 2),
   framed as *testing those*, not a generic "look harder." Instruct it to
   append a new dated section to the existing `<target>-recon.md` rather
   than overwriting prior findings. Increment `reconEscalations` by 1.
   (When the synthesis's top signal is "re-collect/re-test evidence the
   prior pass discarded," the right stage may be `exploit-agent` directly
   rather than another recon pass — use judgment; the goal is to test the
   open assumption, not to always run recon.)
4. Re-dispatch the same stage that was blocked (`exploit-agent` or
   `privesc-agent`), instructing it to read the synthesis block + new recon
   section first and pick up from there rather than re-trying vectors
   already documented as exhausted in its own prior report.
5. If it lands (foothold/escalation achieved), continue the pipeline
   normally from there. If it blocks again, go back to step 1 — re-run the
   synthesis checkpoint (updating the live Assumption Register in place, not
   appending a second one) so each pass looks for something the previous
   pass didn't have, not repeating the same search. Keep looping for as
   many passes as it takes; only stop if a pass itself hits one of the hard
   stops above (e.g. a real usage-limit cutoff reached mid-loop).
