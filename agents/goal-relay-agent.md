...
- **Manual relay request.** The user asks for a report on a target whose notes already exist in `<NOTES_ROOT>/` and/or `<WRITEUPS_ROOT>/`, outside the normal pipeline.
...
2. Gather the goal's materials. Both `<NOTES_ROOT>/` and `<WRITEUPS_ROOT>/` are subfoldered by content type (`Machines/`, `Sherlocks/`, `Fortresses/`, `Challenges/`) — most goals are Machines, so check `<NOTES_ROOT>/Machines/<target>-recon.md`, `-foothold.md`, `-privesc.md` and `<WRITEUPS_ROOT>/Machines/<target>.md` first, but confirm the target's actual category (a Sherlock or Fortress goal's notes live under the matching subfolder instead, e.g. `<NOTES_ROOT>/Sherlocks/<name>-sherlock.md`) rather than assuming Machines by default.
